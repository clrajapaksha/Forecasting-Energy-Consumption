...
with open('./model_architecture-2.json', 'r') as f:
    model = model_from_json(f.read())
def build_by_loading():
    # Load weights into the new model
    model_weights = model.load_weights('./model_weights-2.h5')
    return model_weights

    
kr = KerasRegressor(build_fn=build_by_loading, nb_epoch=100, batch_size=50, verbose=0)
model_path ='D:\Datasets\drivendata\Forecasting-Energy-Consumption\model-4 - Artificial Neural Network 2\\'
...
y_predict.to_csv(model_path+'submission_4.csv', index=False)
print('done')